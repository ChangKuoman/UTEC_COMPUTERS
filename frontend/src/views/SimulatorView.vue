      ...
      <div class="contenedor_app">
        <div v-if="first_part">
          ...
          <div class="SIM_2" v-if="!errors.motherboard">
            ...
            <button class="buttom1" @click.prevent="choose_motherboard">
              CHOOSE ↪
            </button>
          </div>
        ...
        <div v-if="second_part">
          <div class="flex_row">
            <div class="margin_right_20">
              <div class="SIM_3">
                <button class="buttom1" @click.prevent="goBack">↩ BACK</button>
              ...
              <div class="SIM_2" v-if="!errors.component">
                <h2 class="SIM_4">NOW, CHOOSE THE COMPONENTS</h2>
                <div>
                  <div>
                    <div>
                      <h3>
                        YOUR MOTHERBOARD: {{ simulation.motherboard.name }}
                      </h3>
                      <p>
                        MotherBoard Price: S/.
                        {{ simulation.motherboard.price.toFixed(2) }}
                      </p>
                    </div>
                  </div>
                  <form class="Component">
                    <h3>COMPONENTS THAT NEED COMPATIBILITY:</h3>
                    ...
                    <div v-if="lists.peripheral.length">
                      ...
                      <ul class="no-dots">
                        <li
                          v-for="(peripheral, index) in lists.peripheral"
                          :key="index"
                        >
                          <div class="description_name">
                            ...
                            <label for="peripheral"
                              >Name: {{ peripheral.name }}</label
                            >
                          </div>
                          <div class="description_prod">
                            <p>
                              <span class="weight600">Description:</span>
                              {{ peripheral.description }}
                            </p>
                            <p>
                              <span class="weight600">Price:</span> S/.
...
export default {
  ...
  methods: {
    ...
    goBack() {
      (this.first_part = true), (this.second_part = false);
    },
    goHome() {
      this.$router.push("/");
    },
    ...
    simulate() {
      fetch(host + "/simulations", {
        ...
        body: JSON.stringify({
          ...
          components_id: this.total_products
            .filter((product) => product.component_type !== undefined)
            .map((product) => {
              return product.id;
            }),
          token: localStorage.getItem("token"),
      ...
      fetch(host + "/motherboards", { method: "GET" })
        ...
        .then((JsonResponse) => {
          if (JsonResponse["success"] === true) {
            ...
            this.errors.motherboard =
              "We are sorry, there are no motherboards to show";
          } else {
      ...
      fetch(host + "/components", { method: "GET" })
        ...
        .then((JsonResponse) => {
          if (JsonResponse["success"] === true) {
            ...
            this.lists.hdd = components_array.filter(
              (component) => component.component_type === "HDD"
            );
            this.lists.cpu = components_array.filter(
              (component) => component.component_type === "CPU"
            );
            this.lists.psu = components_array.filter(
              (component) => component.component_type === "PSU"
            );
            this.lists.peripheral = components_array.filter(
              (component) => component.component_type === "Peripheral"
            );
            // Above ones needs compatibility w
            const arr = Object.values(
              this.simulation.motherboard.compatibles
            ).map((motherboard) => motherboard.id_component);
            this.lists.ram = components_array.filter(
              (component) =>
                component.component_type === "RAM" && arr.includes(component.id)
            );
            this.lists.ssd = components_array.filter(
              (component) =>
                component.component_type === "SSD" && arr.includes(component.id)
            );
            this.lists.gpu = components_array.filter(
              (component) =>
                component.component_type === "GPU" && arr.includes(component.id)
            );
            this.lists.pc_cooling = components_array.filter(
              (component) =>
                component.component_type === "PC Cooling" &&
                arr.includes(component.id)
            );
          } else {
