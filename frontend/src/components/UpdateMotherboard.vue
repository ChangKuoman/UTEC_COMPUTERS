  ...
  <div>
    ...
    <form v-if="!error_text.length" @change.prevent="clearError()">
      <select v-model="motherboard">
        ...
        <option
          :value="motherboard_e"
          v-for="(motherboard_e, index) in motherboard_list"
          :key="index"
        >
          {{ motherboard_e.name }}
        </option>
      </select>
      <InputText v-model="name" title="NEW MOTHERBOARD NAME" type="text" />
      <InputText
        v-model="price"
    ...
    <div v-if="error_text.length">
      There are no motherboards to update the motherboard
    </div>
  </div>
</template>
    ...
    checkForm() {
      if (this.motherboard === "") {
        ...
        if (this.name === this.motherboard.name) {
          ...
          Object.values(this.motherboard_list)
            .map((motherboard) => motherboard.name)
            .includes(this.name)
        ) {
